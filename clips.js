/* ===========================
   clips.js — CRUD Firebase + límite 6 + tiempo real
   Requiere: funciones.js (exporta db, storage, showToast, setCloudStatus, MAX_CLIPS)
   =========================== */
...
function btnAddSetDisabled(disabled) {
  ...
  if (disabled) {
    els.addBtn.classList.add("is-disabled");
    els.grid?.classList.add("max-reached");
  } else {
    els.addBtn.classList.remove("is-disabled");
    els.grid?.classList.remove("max-reached");
  }
}
  ...
  docs.forEach((snap) => {
    ...
    if (data.url) {
      ...
      if (isVideo) {
        ...
        // Si es URL de Kick/YouTube que no es mp4 directo, no la auto-embebemos para evitar CORS,
    ...
    btnDel.addEventListener("click", async () => {
      ...
      try {
        // Si tiene archivo en storage, borrar
        if (data.storagePath) {
          try {
            await deleteObject(ref(storage, data.storagePath));
          } catch (e) {
            console.warn("No se pudo borrar del storage (continuo con firestore):", e);
          }
        }
...
els.addBtn?.addEventListener("click", () => {
  if (state.currentCount >= MAX_CLIPS) {
    showToast("Solo se permite un máximo de 6 clips. Elimina uno para agregar otro.");
    return;
  }
  openModal("add");
});
...
els.form?.addEventListener("submit", async (e) => {
  ...
  // Límite en alta (no bloquear edición)
  if (!state.editingId && state.currentCount >= MAX_CLIPS) {
    showToast("Solo se permite un máximo de 6 clips. Elimina uno para agregar otro.");
    return;
  }

  // Si hay archivo, subimos al Storage
  let storagePath = null;
  if (file) {
    try {
      const path = `clips/${Date.now()}_${file.name}`;
  ...
  try {
    if (state.editingId) {
      // Si estamos reemplazando archivo, borra el anterior (si lo había)
      const id = state.editingId;
      const docRef = doc(db, "clips", id);

      // Para borrar anterior storage, necesitamos conocerlo:
      // Traemos el documento actual para revisar storagePath existente
      // (usamos una lectura rápida con getDocs del query filtrado por ID)
      // Nota: Firestore no tiene getDoc en esta import, preferimos hacerlo arriba si quieres optimizar.
      // Aquí simplificamos: si el usuario subió un nuevo archivo (storagePath != null)
      // asumimos que quiere reemplazar el anterior y tratamos de borrarlo si lo conocemos en snapshot realtime.
      // Como onSnapshot no está aquí, no tenemos el anterior, así que no borramos previo en edición salvo que
      // implementes una lectura puntual. Si necesitas borrado estricto del anterior, avísame y lo añado.
      ...
      await addDoc(clipsRef, {
        title,
        url: url || "",          // puede ser vacío si no puso URL ni archivo (no recomendado)
